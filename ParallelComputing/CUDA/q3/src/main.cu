...
int main(int argc, char *argv[])
{
    ...
    int total_blocks = blocks * n ;

    int reduce_block_size = block_size / 2;

    int per_row_blocks = blocks;

    int reduce_blocks = per_row_blocks * n;

    int reduce_size = size * per_row_blocks;

    float *host_a = (float *) malloc(size * n);
    float *host_b = (float *) malloc(size);
    float *host_c = (float *) malloc(reduce_size);
    float *host_result = (float *) malloc(size);
    ...
    statusC = cudaMalloc(&dev_c, reduce_size);
    ...
    print_vec("C vector:\n", host_c, reduce_blocks);

    start_timer(&kernel_timer);
    multiply <<< total_blocks, block_size >>>(dev_a, dev_b, blocks);
    reduce <<< reduce_blocks, reduce_block_size >>>(dev_a, dev_c, n);
    stop_timer(&kernel_timer);
    check_error( cudaGetLastError(), "Error in kernel.");

    check_error(cudaDeviceSynchronize(), "");
    start_timer(&h2d_timer);
    statusC = cudaMemcpy(host_c, dev_c, reduce_size, cudaMemcpyDeviceToHost);
    ...
    print_vec("C vector:\n", host_c, reduce_blocks);
    for (index = 0; index < n; index++) {
        for (innerindex = 0; innerindex < per_row_blocks; innerindex++) {
            *(host_result + index) += *(host_c + innerindex + index * per_row_blocks);
        }
    }

    print_vec("Results vector:\n", host_result, n);
    print_results(
        n,
        n,
        n,
